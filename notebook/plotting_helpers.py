...
def mollweide_projection(l, b, l2, b2, title, bmin, bmax, nside, smooth, q=[0], **kwargs):
    ...
    if len(l2) > 0:
        l2_arr = ((np.asarray(l2) + 180) % 360) - 180
        b2_arr = np.asarray(b2)
        wrap_idx = np.where(np.abs(np.diff(l2_arr)) > 180)[0] + 1
        for seg_l, seg_b in zip(np.split(l2_arr, wrap_idx), np.split(b2_arr, wrap_idx)):
            hp.newprojplot(theta=np.radians(90-seg_b), phi=np.radians(seg_l),
                            marker='', color='black', lw=1.5)
        hp.newprojplot(theta=np.radians(90-b2_arr[-1]), phi=np.radians(l2_arr[-1]),
